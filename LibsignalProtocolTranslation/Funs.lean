...
def identity_key.IdentityKey.is_same_account
  ...
  then ok false
    ...
    let s ← libsignal_core.address.ProtocolAddress.name self_protocol_address
    let o ← libsignal_core.address.ServiceId.parse_from_service_id_string s
    match o with
    | none => ok false
    | some self_service_id =>
      let s1 ←
        libsignal_core.address.ProtocolAddress.name other_protocol_address
      let o1 ←
...
def protocol.SignalMessage.serialize_addresses
  ...
  := do
  let s ← libsignal_core.address.ProtocolAddress.name sender
  let o ← libsignal_core.address.ServiceId.parse_from_service_id_string s
  let cf ← core.option.Option.Insts.CoreOpsTry_traitTry.branch o
  match cf with
  | core.ops.control_flow.ControlFlow.Continue val =>
    let s1 ← libsignal_core.address.ProtocolAddress.name recipient
